...
joint.shapes.dialogue.BaseView = joint.shapes.devs.ModelView.extend({
    ...
    updateBox: function() {
        ...
        this.$box.css({
            ...
            width: bbox.width,
            height: bbox.height,
            left: bbox.x * graphScale,
            top: bbox.y * graphScale,
        });
    },
...
joint.shapes.dialogue.TextareaView = joint.shapes.dialogue.BaseView.extend({
    ...
    initialize: function() {
        ...
        ta.on('mousedown click', function(evt) {
            evt.stopPropagation();
        });
    ...
    updateBox: function() {
        ...
        this.$box.css({
            width: bbox.width,
            height: bbox.height + Math.min(nameField[0].scrollHeight, textarea_max_h),
        });
    },
});
...
joint.shapes.dialogue.Random = joint.shapes.devs.Model.extend({
    defaults: joint.util.deepSupplement({
            ...
            attrs: {
                '.outPorts circle': {
                    unlimitedConnections: ['dialogue.Choice', 'dialogue.Text'],
                }
            },
        },
        joint.shapes.dialogue.Base.prototype.defaults
...
joint.shapes.dialogue.Note = joint.shapes.devs.Model.extend({
    defaults: joint.util.deepSupplement({
            ...
            attrs: {
                '.outPorts circle': {
                    unlimitedConnections: ['dialogue.Plot', 'dialogue.Note', 'dialogue.StartConv',
...
joint.shapes.dialogue.BranchView = joint.shapes.dialogue.BaseView.extend({
    ...
    initialize: function() {
        joint.shapes.dialogue.BaseView.prototype.initialize.apply(this, arguments);
        this.$box.find('input.name').on('mousedown click', function(evt) {
            evt.stopPropagation();
        });
    ...
    updateSize: function() {
        ...
        this.model.set('size', {
            width: 200,
            height: 100 + Math.max(0, (this.model.get('outPorts').length - 1) * height)
        });
    },
});
...
joint.shapes.dialogue.SetView = joint.shapes.dialogue.BaseView.extend({
    ...
    initialize: function() {
        ...
        this.$box.find('input.value').on('mousedown click', function(evt) {
            evt.stopPropagation();
        });
        this.$box.find('input.name').on('change', _.bind(function(evt) {
            this.model.set('name', $(evt.target).val());
...
var paperScale = function(s) {
    ...
    container.style.transform =
        'scale(' + graphScale + ')' +
        'translate(' + graphScale * 100 + ')';
    scrollSize = {
        height: container.scrollHeight,
        width: container.scrollWidth
    };
    var sx = container.scrollLeft
    sy = container.scrollTop;
    var remap = function(value, low1, high1, low2, high2) {
        return low2 + (high2 - low2) * (value - low1) / (high1 - low1);
    }
    container.scrollTo(remap(sx, 0, oldScrollSize.height, 0, scrollSize.height),
        remap(sy, 0, oldScrollSize.width, 0, scrollSize.width));
};
